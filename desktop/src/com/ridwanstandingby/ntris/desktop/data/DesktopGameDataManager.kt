...
class DesktopGameDataManager : GameDataManager() {
    ...
    private inline fun <reified T> setValue(key: String, value: T) {
        ...
        val newFileString = StringBuilder().apply { newLines.forEach { appendLine(it) } }.toString()
        File(PREFS_FILE_NAME).writeText(newFileString)
    }

    private inline fun <reified T> generateNewLines(
        key: String,
        value: T,
    ...
    private fun isLineForPreference(line: String, key: String) =
        line.contains(key) && line.contains(PREFS_DELIMITER)
